...
async function fetchAllRows() {
  ...
  for (let offset = 0; ; offset += PAGE) {
    const { data, error } = await supabase
      ...
      .select('month,name,cat1,cat3,contracts,qty')
...
function computeAnalysis(rows) {
  ...
  // 상품명 그대로 묶으면 "복사용지"/"반비 복사용지"/"복사용지(꿈드래)A4"/"위드페이퍼 복사용지"처럼
  // 브랜드만 다른 같은 품목이 전부 따로 카운트돼서 실제로는 훨씬 꾸준한 품목이 순위에서 쪼개져 보인다.
  // 그래서 상품명이 아니라 1차+3차 카테고리(품목 단위)로 묶고, 합쳐진 상품명들은 variants에 남겨둔다.
  ...
  for (const r of rows) {
    ...
    const itemKey = `${r.cat1}|||${r.cat3}`
    if (!byProduct.has(itemKey)) {
      byProduct.set(itemKey, { cat1: r.cat1, cat3: r.cat3, variants: new Set(), months: new Set(), totalContracts: 0, totalQty: 0, byCalMonth: {} })
    }
    const p = byProduct.get(itemKey)
    ...
    if (!byCategory.has(r.cat1)) byCategory.set(r.cat1, { cat1: r.cat1, totalContracts: 0, totalQty: 0, products: new Set() })
    const c = byCategory.get(r.cat1)
    c.totalContracts += r.contracts
    c.totalQty += r.qty
    c.products.add(r.name)
  }

  const products = [...byProduct.values()]

  const categoryRanking = [...byCategory.values()]
    .map((c) => ({ cat1: c.cat1, totalContracts: c.totalContracts, totalQty: c.totalQty, productCount: c.products.size }))
  ...
  const seasonalCandidates = products
    ...
    .map((p) => {
      ...
      return {
        cat1: p.cat1, cat3: p.cat3, variantCount: p.variants.size, variants: [...p.variants].sort(), topMonth,
  ...
  return { totalMonths, totalRows: rows.length, steady, byCalendarMonth, calendarBars, monthCounts, bulkDeals, categoryRanking }
}
