...
import { getCachedAnalysis, listAnalysisYears, MONTH_LABEL } from '../lib/s2bAnalysis'
...
export default function AnalysisPage({ scope, years, data }) {
  ...
  const { totalMonths, totalRows, steady, byCalendarMonth, calendarBars, updatedAt } = data
    ...
    <>
      <div className="detail-header" style={{ padding: '0 0 16px' }}>
        ...
        <p style={{ color: 'var(--text2)', fontSize: 14, marginTop: 6 }}>
          {scope === 'all' ? `누적 ${totalMonths}개월` : `${scope}년 ${totalMonths}개월`} · {totalRows.toLocaleString()}건 기준(계약건수=반복수요 지표).
          마지막 분석 갱신: {new Date(updatedAt).toLocaleString('ko-KR')}
        </p>
      </div>
      ...
      <section style={{ marginBottom: 40 }}>
        <div className="section-title">🗓️ 캘린더월별 판매 흐름 <span>({scope === 'all' ? '전체 연도 합산' : `${scope}년`} 계약건수)</span></div>
        <div className="card" style={{ cursor: 'default' }}>
          {calendarBars.map((b) => (
            <div className="bar-row" key={b.key}>
              <div className="bar-label">{b.label}</div>
              <div className="bar-track">
                <div className="bar-fill" style={{ width: `${Math.max(b.pct, 4)}%` }}>{b.total.toLocaleString()}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ marginBottom: 40 }}>
        <div className="section-title">🔥 꾸준히 잘 팔리는 상품 TOP {steady.length} <span>(등장 개월수 · 평균 계약건수 기준)</span></div>
        <div className="data-table-wrap">
          <table className="data-table">
            ...
            <tbody>
              ...
              {steady.length === 0 && (
                <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--text3)' }}>등장 개월수 5개월 이상인 상품이 없어요.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <section>
        <div className="section-title">🌱 월별로 몰리는 시즌 특수 상품 <span>(특정 달에 계약이 집중된 상품)</span></div>
        <div className="grid-auto">
          {Object.keys(MONTH_LABEL).map((key) => (
            <div className="card" key={key} style={{ cursor: 'default' }}>
              ...
              {byCalendarMonth[key].map((p, i) => (
                <div key={p.name + i} style={{ fontSize: 12, marginBottom: 6, display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <span style={{ color: 'var(--text3)', flexShrink: 0 }}>{p.totalContracts.toLocaleString()}건</span>
                </div>
              ))}
            </div>
          ))}
